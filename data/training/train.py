...
Generates labelled samples covering all 11 features and trains a LogisticRegression model.
...
def generate_synthetic_data(n_samples: int = 300):
    """
    Build a labelled synthetic dataset covering all 11 features.
    ...
    Feature index mapping (must stay in sync with FEATURE_NAMES):
      ...
      9: punycode_detected       (SEC-5)
      10: injection_in_url       (FIX-13) <- NEW

    Fraud prior:  ~30% cloaking=1, ~10% punycode=1, ~10% injection_in_url=1.
    Legit prior:  cloaking=0, punycode=0, injection_in_url=0 always.

    Returns X (float ndarray shape [n_samples, 11]), y (int ndarray shape [n_samples]).
    """
    assert len(FEATURE_NAMES) == 11, (
        f"FEATURE_NAMES length mismatch: expected 11, got {len(FEATURE_NAMES)}. "
    ...
    for _ in range(n_legit):
        ...
        injection      = 0   # legit QR codes never embed injection phrases

        X.append([domain_age, redirect_count, is_typo, bulletproof,
                  ssl_age, vpa_mismatch, blacklisted, typo_dist, cloaking, punycode, injection])
        y.append(0)
    ...
    for _ in range(n_fraud):
        ...
        # ~10% of fraud samples embed prompt-injection phrases in the URL
        injection   = np.random.choice([0, 1], p=[0.9, 0.1])
...
def main() -> None:
    """Train and persist the risk classifier with 11 features."""
    X, y = generate_synthetic_data(300)

    assert X.shape[1] == 11, f"Feature vector width mismatch: expected 11, got {X.shape[1]}"
